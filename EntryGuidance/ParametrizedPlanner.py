...
def OptimizeSRP():
    ...
    if 1:
        # bounds = [(0,150),(100,250),(88,90),(0,3)]
        # bounds = [(0,110),(50,140),(100,160),(70,90),(0,20)]
        bounds = [(0,150),(100,160),(70,90),(0,20)]
        # bounds = [(0,250),(100,400),(100,550)]
        for DR in np.linspace(750,795,20):
            sol = differential_evolution(SRPCost, args = (sim,x0,None,DR), bounds=bounds, tol=1e-2, disp=False, polish=False)
            print("DR = {} km, sol = {}, J = {} km".format(DR, sol.x, sol.fun))
    else:
    ...
    # bankProfile = lambda **d: profile(d['time'], switch=sol.x[0:2], bank=[np.radians(sol.x[2]),-np.radians(sol.x[2]),np.radians(sol.x[3])],order=2) # No reversals
    #
    # print sol.x
...
def SRPCost(p, sim, x0, sample=None, dr_target=None):

    # dr_target = 900
    if dr_target is None:
        w=0
    else:
        w=1
    cr_target = 0
    ...
    n=2 # or 3
    J = checkFeasibility(p[0:n])
    if J > 300:
        return J

    # bankProfile = lambda **d: profile(d['time'], switch=p, bank=[np.radians(-30),np.radians(75),np.radians(-75),np.radians(30)],order=2)
    bankProfile = lambda **d: profile(d['time'], switch=p[0:2], bank=[np.radians(p[2]),-np.radians(p[2]),np.radians(p[3])],order=2) # two switches
    # bankProfile = lambda **d: profile(d['time'], switch=p[0:3], bank=[-np.radians(p[4]),np.radians(p[3]),-np.radians(p[3]),np.radians(p[4])],order=2) # three switches
    ...
    J = -hf + (w*(dr_target-dr)**2 + 1*(cr_target-cr)**2)**0.5
