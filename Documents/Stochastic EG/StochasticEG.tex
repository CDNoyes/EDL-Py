...
% Pose the full problem, then state how MSL tackled it (and any assumptions), then give our approach (whatever that is...). Then give details, and results.
	...
	Broadly speaking, the goal of entry, descent, and landing is to deliver a spacecraft safely to the planet surface. The modern era of Mars EDL, beginning with MSL in 2012, utilizes hypersonic entry guidance to reduce the landing footprint and deliver ever-increasing payload mass. The initial state of the vehicle at entry interface will differ from the planned entry (delivery error), and this state will not be known exactly onboard the vehicle (knowledge error). The onboard GNC system will have imperfect models of the planetary atmosphere and the aerodynamics of the vehicle.
