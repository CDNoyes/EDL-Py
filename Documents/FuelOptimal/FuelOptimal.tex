...
\begin{abstract}
Powered descent already plays an important role in current Mars EDL, and its importance will grow as larger and heavier payloads are required to be delivered. Much research has been devoted to solving powered descent (or powered approach) problems with various constraints and objectives, including fuel minimization. Far less energy has been spent in developing appropriate entry guidance strategies that coordinate with the powered descent guidance algorithm to deliver the entry vehicle to favorable ignition conditions. We begin by demonstrating significant deficiencies in the current state of the art approach. Our approach extends a predictor-corrector guidance through the subsequent powered phase, and alters the objective of the entry guidance to minimize the fuel required to bring the vehicle to desired conditions via propulsion. 
\end{abstract}
%\section{Notes}
...
%
%\textbf{TODO:} Diagram showing full predicted entry trajectory, highlight the portion checked for SRP, show the ignition condition. Note that possible points are trimmed by velocity, range to go, and altitude Perhaps also reference Guru/Joel doc for phase definitions, and show phases. Unpowered entry flight, powered approach (or deceleration?), powered descent includes potential final divert vertical and vertical phase. 
%
%A divert is likely necessary because SRP begins far from the target where hazards may not be known. 
%
...
Powered descent played a role in safely landing MSL's Curiosity rover, and will also be utilized on Mars2020, and Mars Sample Return campaign. In future missions, supersonic retropropulsion may be used to land larger, heavier vehicles relative to the current generation of landers, and powered descent will play an even more significant role in the EDL process. There exist many guidance approaches to the powered descent problem, with many focusing on fuel optimality, including GFOLD and other convex optimization methods, polynomial guidance, indirect methods, etc. In brief, there is no shortage of ways to tackle the problem of decelerating via propulsion. 
...
An adaptive trigger for termination of unguided flight was demonstrated to provide massive benefits over triggering at a fixed state variable such as downrange, velocity, or energy. The issue of monotonicity in the fuel required was dealt with by solving a related powered descent optimal control problem. Despite presenting a predictor-corrector approach in which a prediction of the terminal entry state is available and being used to modulate the bank angle, only the current vehicle state is checked for triggering. Our approach to the triggering mechanism differs in that we utilize the predicted trajectory. This allows us to avoid the issue of monotonicity of predicted fuel consumption, and also guarantees we find the global minimum. 

A fundamental difference of retropropulsion-based EDL from parachute-based EDL architectures, even those that utilize a vertical powered descent phase with potential for a divert maneuver prior, lies in the set of desirable states at the termination of the entry phase. Traditionally, the entry objective is range control with lateral control as a secondary objective, and the two problems can be neatly decoupled. In determining the powered descent ignition state, however, both the lateral and longitudinal motion is required. This brings to bear two important considerations: the chosen parametrization of the bank angle, and the objective of bank angle modulation. 

In the work by Ping Lu, the bank angle is still modulated to perform range control, and utilizes a profile linear in energy with one parameter. Lateral logic to determine bank reversals is applied separately after solving for the bank angle magnitude. In our approach, lateral and longitudinal motions are not decoupled, and thus the minimum number of parameters we consider in our parametrization is two.

Additionally, the proposed entry guidance approach utilizes knowledge of the powered guidance approach to be used in the subsequent phase, making the two phases linked by more than just the adaptive trigger. Instead, the bank angle is actively modulated to reduce predicted fuel consumption rather than achieve a range objective, thereby coordinating to produce superior results. 
...
%\begin{enumerate}
%\item Integrate the estimated vehicle state forward using the current bank angle parametrization until the target range is reached
%\item Determine the optimal ignition state along the unpowered entry trajectory
...
\begin{figure}[h!]
		...
		\caption{Bank angle required to achieve a fixed downrange versus the terminal velocity corresponding to various terminal energies at zero altitude.}
...
A ``standard" predictor-corrector entry guidance algorithm determines the bank angle required to achieve a downrange distance at a fixed energy level. 
Figure~\ref{fig_bank_vs_vel} shows the bank angle required to achieve a targeted downrange distance as a function of the terminal velocity. Note that the simulations are terminated based on energy, using a zero altitude target, i.e., E = f(h, v), but for fixed altitude of zero,  E=f(0, v) there is a 1-to-1 correspondence between energy and velocity.
 
To determine the optimal fuel contours for a chosen parametrization of the bank angle during entry flight, we begin by simulating trajectories with a constant bank angle and one bank reversal over the space $[0^\circ,\,90^\circ]\times [2000,\, 4000]$ m/s. This parametrization is motivated by simplicity, and is sufficient to illustrate the point of this section. Along each resulting trajectory, the point with the lowest fuel consumption required for a pinpoint landing is determined, and the fuel required is recorded. Figure~\ref{fig_contour} depicts the results, in which it is evident that only a narrow band of bank angles of roughly $4^\circ$ can achieve the desired pinpoint landing at all, even with an \textit{optimally timed} bank reversal and an \textit{optimally triggered} powered descent initiation. Furthermore, only about half of that already narrow range produces trajectories with an acceptable fuel consumption.

Comparing the two figures, we see that although bank angles from $10^\circ$ to $30^\circ$ can satisfy the range requirement at different energies, the powered descent problem places more stringent requirements on the entry trajectory. This highlights the absolute necessity of coupling the two phases and coordinating their guidance approaches; failure to do so results in significant increases in fuel required, or infeasible situations in the worst cases.

Problem details: 8500 kg vehicle, BC = 366 $\mathrm{kg/m}^2$, S = 15.8 m$^2$, L/D = 0.24, EFPA = $-16.9^\circ$, DR~=~753.7 km, $ I_{sp} $ = 295 s, $T_{\max} = 595$ kN


\section{Proposed Entry Guidance}

The heart of the proposed guidance approach is to extend the trajectory prediction through to the following powered descent phase. This allows us to alter the entry guidance objective from range control to fuel minimization in the predictor-corrector. Notice that although fuel minimization is the entry guidance objective, any powered descent guidance algorithm may be used in the powered phase so long as an estimate of the fuel required is given by the algorithm, and this estimate is continuous with respect to the initial state. Thus, although in the examples of this work we have coordinated the powered descent algorithm to also be fuel-optimal in nature, there exist many alternatives such as energy-optimal guidance or non-optimization based approaches such as polynomial guidance. 

We further assume that the chosen powered descent method incorporates any necessary problem constraints, because the entry guidance algorithm does not make use of the actual powered descent trajectory (nor does it check for satisfaction of constraints) but instead only the associated fuel cost. 
...
\right.
\end{equation}
and we used $\sigma_1 = 90^\circ$, $-\sigma_2 = \sigma_3 = 15^\circ$.
...
The numerical integration of the entry state is carried out with respect to downrange distance to the target. This has several advantages over common alternatives. First, although energy or velocity could be used, a sufficiently low choice must be made, but time spent integrating below the reachable set of the vehicle is wasted, and the choice is problem/vehicle specific. Altitude would also be a natural choice since the minimum ignition altitude would serve as the terminal integration condition, but like energy and velocity this is problem dependent, and additionally altitude is not a strictly monotonic variable in the case of lofting trajectories. Using range to go as the independent variable allows the terminal integration condition to be zero, independent of the problem specifics, with very little wasted integration time since the optimal ignition state is generally close to the target. 

Once the prediction is generated, several constraints are applied to filter the trajectory down to points that should be checked for their predicted fuel consumption. Because this step could be costly (depending on the chosen powered approach guidance), these constraints greatly reduce the number of calls to the PA guidance required. Figure~\ref{fig_ignition} depicts this scenario including a portion of the trajectory already flown, the entry flight prediction, constraints, target, and predicted optimal ignition point.

\begin{figure}[h!]
	\centering
	\includegraphics[width=1\textwidth]{H_Vs_S_625} 
	\caption{Only the portion of the entry trajectory satisfying constraints on velocity, altitude, and range to go are searched for the fuel optimal ignition state.}
	\label{fig_ignition}
\end{figure}
...
We require that the chosen powered descent guidance approach admits continuous fuel requirements, and the entry equations are also smooth for smooth bank angle inputs. Thus we expect that even when optimizing the ignition condition along a trajectory rather than use a fixed state variable such as velocity or energy, the objective function should be well behaved. One question is whether several isolated minima exist, or only a single global optimum. 
...
\subsection{Feedback: Implicit vs Explicit}
A standard feature of the predictor-corrector approach is the implicit generation of feedback by repeated calls with the current state, not unlike model predictive control. However, noting that the prediction and associated computations may be lengthy computationally, tracking the latest predicted trajectory has been shown to be effective while reducing computational burden. This approach has been taken here, where a low number of calls to the predictor-corrector are made, and the vertical L/D of the most recent trajectory is tracked by modulating the bank angle. This essentially decouples the guidance algorithm into two parts, the trajectory resulting from the predictor-corrector mitigates delivery errors and errors accumulated during flight, while the tracking component seeks to reduce the accumulation of errors in flight. 

\subsection{Entry State to Powered Descent State Conversion}
Given an entry state of the form $[r,\, \theta,\, \phi,\, v,\, \gamma,\, \psi]$, the corresponding SRP state is determined by assuming the current heading $\psi$ defines the downrange distance. For a fixed target $(h_T,\, \theta_T,\, \phi_T)$, the range to go, and crossrange to target are computed. The SRP state is given by $[rtg,\, |cr|,\, r-r_p-h_T, v\cos\gamma, 0, v\sin\gamma, m_0]$. First, note that the absolute value of the crossrange is used; this gives a denser table of tabulated solutions by exploiting symmetry. Similarly, because the downrange direction is defined by the heading angle, the cross track velocity is always zero, allowing the use of 5D interpolation rather than 6D, again allowing the same number of solutions to more densely cover the input space. 

\section{Example}


\section{Conclusion}
Work to go includes a more efficient method of optimization and investigation of different bank angle parametrizations. 
