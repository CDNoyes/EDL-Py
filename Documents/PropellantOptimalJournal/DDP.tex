...
\end{align}
%TODO: L/D control, reference Eq.3 in MSL design paper
...
%TODO: Metrics that are not strongly affected by the bank angle, including acceleration load and heat flux are not included in the formulation. However, because the EFPA is a first order driver of the metrics, if the EFPA were included then so too would we reconsider incorporating these as constraints. Simply say that we are not explcitiyl accounting for these. MSL experience has shown they are primarily dependent on the EFPA. 

%TODO: Mean/target DR is unconstrained with the idea that the entry point will be adjusted 
