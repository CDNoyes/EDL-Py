...
% In light of the fact that entry guidance performance is assessed by simulation and quantified using statistics, the our approach is to directly optimize a performance measure based on those statistics. To accomplish this, a robust optimal guidance problem is formulated accounting for the guidance control law and parametric uncertainty in the entry dynamics. In order to make the problem tractable, we restrict the guidance control law to be a static state feedback law, and we use the unscented transform to approximate the mean and covariance of the state distribution as it evolves under the nonlinear closed-loop dynamics. We expect that by properly weighting the deviations, landing at the same elevations as MSL can be achieved with better range control accuracy. 
...
%We remark that the magnitude of these dispersions
...
% Compare to the optimal altitude of case 1.
% We remark that the high ballistic coefficient and constant L/D are not compatible with a 6 km minimum altitude deployment required for current generation missions; additional solutions suggest a constant 0.28 L/D is sufficient for a BC up to 180 or so, and alternatively a constant L/D = ? is required for a vehicle with \beta=225. 
...
%Scaling this profile to higher L/D values would produce higher deployment altitudes.
\begin{table}[h!]
	\centering
...
%Robust optimal sols have good performance in the mean compared to the deterministic/nominal case. 
