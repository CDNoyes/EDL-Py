...
%The differences are: 
%how the ref traj is generated, 
%the use of feedback control to track the reference traj both in the range controller and in the design of the ref traj, 
%and final objective which is a weighted combination maximizing altitude and minimizing altitude dispersions and range dispersions. 

%The design process is substantially simplified by accounting for uncertainty in the generation of the reference trajectory; in particular, Monte Carlo simulations with 100’s or 1000’s of candidate reference trajectories (ref Mendeck/Craig) are avoided in the design process. Monte Carlo simulation is used post-design to validate and characterize the performance of the resulting guidance. (And as you already have, say accounting for uncertainty is made feasible by using a combo of the unscented transform and DDP.) No change is considered for the lateral controller.

% Overview and proposed benefits:
Like MSL and Mars 2020, we consider the guidance approach as bank angle modulation using reference trajectory-based feedback control. In this work we present an alternative to the range controller used by MSL and Mars 2020. Briefly, our approach differs by posing the design of the reference trajectory as the solution to a robust optimal control problem, in which the objective is a weighted combination maximizing altitude and minimizing altitude dispersions and range dispersions. The design process is substantially simplified by accounting for uncertainty in the generation of the reference trajectory; in particular, Monte Carlo simulations with numerous candidate reference trajectories (\cite{MSL_EDL2}) are avoided in the design process. Monte Carlo simulation is used post-design to validate and characterize the performance of the resulting guidance. No change is considered for the lateral controller.
...
\begin{align}
	\frac{L}{D}u(v,\state) &= \frac{L}{D}_{\mathrm{ref}}\ur(v) + \frac{L}{D}\delta u(v,\state) \label{eq_control_rearranged}
\end{align}
...
\begin{align}
	...
	\end{matrix}
	\right]
\end{align} and augmenting the entry dynamics with trivial dynamics
\begin{align}
	\state' = \left[
\begin{matrix}
	\state'_{\mathrm{lon}} \\
...
\right]
\end{align}
%This augmented state vector yields additional sigma points. 

%In EDL simulations, uncertainties may be modeled as stochastic processes, as constant offsets, as functions of the state or independent variable, or as some combination thereof. Our use of constant uncertainty models allows the use of the unscented transform to propagate estimates of the mean and covariance. The standard deviation of each constant uncertainty can be chosen sufficient large to cover any of the aforementioned uncertainty models, potentially at the cost of introducing conservatism to the solution (i.e. overestimating the variance of state variables or other key quantities)

As a result of considering uncertainty, the state variables are random variables and the goal is to optimize a distribution of trajectories rather than a single trajectory. The mean, or expected value, of a random variable, $x$ is denoted by $\bar{x}=\E{x}$, and its standard deviation by $\sigma_{x}$. A nominal trajectory is one in which all uncertain quantities are set to their mean value. For nonlinear systems, including entry dynamics, the nominal trajectory is in general not equal to the mean trajectory except at the initial time. The initial state vector is assumed to be governed by a distribution with known mean $\bar{\state}_0$ and covariance matrix $\cov_0$.
In this work, we assume the initial state is normally distributed, expressed as 
\begin{align}
\state(v_0)\sim N(\bar{\state}_0,\,\cov_0)\label{eq_IC},
\end{align}
but the approach also applies to non-normal distributions. Even if the initial state is normally distributed, in general the state distribution will evolve into a non-normal distribution under the nonlinear entry dynamics. The parameter variations are constant fractions applied to the mean parameters such that
\begin{align}
	\param(v,\state) = \bar{\param}(v,\state)(1 + \delta\param) \label{eq_param_unc}
%	\delta\param\sim N(\mathbf{0},\cov_{\param})
\end{align}
This formulation allows the mean parameters to vary along the trajectory, while the parameter variations around the mean are constants. An example where this is necessary is the $L/D$ profile, which is often modeled as a function of Mach number. We model the parametric uncertainty as in Eq.~(\ref{eq_param_unc}), but an additional possibility, that may be treated with no change to the solution method, is 
% Regarding atmospheric density variations, we consider a constant percentage In theory, even more complex models can be treated this way. For example, in models of the atmosphere (such as MarsGRAM\cite{MarsGRAM2010}), the atmospheric density may be modeled such that both the mean and standard deviation vary with altitude. Such a model can be treated by assuming that a constant fraction of the maximum variation is applied throughout the trajectory
\begin{align}
	\param(v,\state) = \bar{\param}(v,\state) + \delta\param\sigma_{\param}(v,\state)
\end{align}
in which the standard deviation is also allowed to vary along the trajectory. This could be used to represent, for example, an atmospheric density model in which both the mean and standard deviation vary with altitude, such as MarsGRAM \cite{MarsGRAM2010}. As an additional remark, the guidance performance can be assessed using different, higher fidelity models relative to those used in the optimization/design process. 
...
the simulations begin at a lower altitude where the atmosphere is sufficiently dense for bank angle control to be effective and the vehicle velocity magnitude is strictly decreasing. This is similar to the way MSL and Mars 2020 flew a pre-bank angle until active range control begins. The mean entry state, used in all of the experiments, is $x_0 = [54.5\,\mathrm{km},\,-11.5^{\circ}, 0\,\mathrm{km}]^T$ at $v_0 = 5525$ m/s. The initial state uncertainty around the mean is $\sigma_h = 2.5$ km, $\sigma_{\gamma} = 0.25^{\circ}$, $\sigma_s = 10$ km. Uncertainty is also modeled in the $L/D$ ratio, ballistic coefficient, and atmospheric density with $\sigma_{L/D} = 5\%$, $\sigma_{\beta} = 5\%$, $\sigma_{\rho} = 6.66\%$. With uncertainty in each state variable as well as in three model parameters, there are six uncertainties under consideration and the unscented transform yields 13 sigma points, resulting in an augmented state vector with 78 state variables. The terminal entry velocity is $v_f=460$ m/s. The problem is discretized into $N=250$ fixed velocity increments, and the control is assumed to be constant on each interval. Four Euler integration steps are taken over each velocity step. The value of $ N $ and the number of integration steps were chosen by validating the terminal altitude and downrange distance against adaptive stepsize integration using MATLAB's \textit{ode45} function. The acceptable error tolerance across all sigma points was 100 m. The unscented transform tuning parameter $\alpha=15$ in all examples. This value approximately minimizes the UT-estimated downrange distance standard deviation for our guess trajectory, i.e. the initial trajectory with which DDP is initialized. 

\subsection*{Reference Trajectory Comparison}
In this subsection we compare the performance of the proposed range controller using six different reference trajectory-control pairs. For all six cases, the controller, Eq.~(\ref{eq_control}), is used with the same feedback gains and feedback control limits, Eq~(\ref{eq_control_bounds}). The first three cases generate a reference trajectory and control by solving the optimal control problem $\min J = -h(v_f)$ subject to nominal longitudinal dynamics with different limits on the reference control to preserve varying amounts of margin for feedback. The remaining three cases use the method proposed in this paper to generate the reference trajectory and control. Monte Carlo simulations are used to assess the guidance performance.
...
We have presented an alternative approach to range control for Mars entry guidance. The entry guidance problem is posed as a robust optimal control problem with uncertain initial state and parameters. The statistics required to evaluate the objective function are estimated with sample trajectories computed via the unscented transform. Differential dynamic programming is demonstrated to be an effective way to solve the resulting large-scale optimal control problem. By augmenting the mean terminal altitude maximization objective with minimizing the standard deviations of altitude and downrange distance, reference entry trajectories robust to the modeled uncertainties are generated. Monte Carlo simulations verified the performance of the reference trajectories and controls. The results using reference trajectories generated by the proposed approach showed clear improvement over reference trajectories generated by optimal control solutions that do not account for uncertainty. 
...
%TODO: Work to go includes testing (and possibily optimizing) with lateral guidance? Possible to constrain the mean crossrange to zero? Planned reversals versus corridor? 
