\chapter{Entry Vehicle and Environment Models}
...
%TODO: Talk about orbital insertion/delivery as the reason for initial state dispersions 
...
%In practice, some models may apply, e.g., a 3$\sigma$ uncertainty early in the trajectory that decays toward the mean value later; in this case our model would assume a 3$\sigma$ value over the entire trajectory. As such, this can result in an overestimate of the covariance matrix. 
...
%Note that  except at the initial time
...
%It can also be used to consider location (latitude and longitude) dependent atmosphere models.  
%The effect of overestimating the covariance matrix during trajectory optimization is increased robustness. ?
